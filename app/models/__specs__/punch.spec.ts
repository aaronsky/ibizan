...
const org = new Organization(MockConfig.team);
org.projects = [
...
describe('Punch', () => {
    ...
    describe('.parse(organization, user, command, mode, timezone?)', () => {
        ...
        it('should return if user and command are not defined', () => {
            expect(Punch.parse(org, null, 'a dumb command')).to.be.null
            expect(Punch.parse(org, this.user, null)).to.be.null
        });
        ...
        it('out without arguments', () => {
            const punch = Punch.parse(org, this.user, 'out', 'out');
        ...
        it('in with an existing project', () => {
            const punch = Punch.parse(org, this.user, `in #${this.projectName}`, 'in');
            expect(punch).to.have.property('mode', 'in');
            expect(punch).to.have.deep.property('times[0]');
        ...
        it('out with two existing projects', () => {
            const punch = Punch.parse(org, this.user, `out #${this.projectName} #${this.alternateProject}`, 'out');
            expect(punch).to.have.property('mode', 'out');
            expect(punch).to.have.deep.property('times[0]');
            expect(punch).to.have.deep.property('projects[0]');
            expect(punch).to.have.deep.property('projects[0].name', this.projectName);
        ...
        it('in at a time with a period marker and no space between', () => {
            const punch = Punch.parse(org, this.user, 'in 9:15pm', 'in');
            expect(punch).to.have.property('mode', 'in');
            expect(punch).to.have.deep.property('times[0]');
        ...
        it('in at a time with a period marker', () => {
            const punch = Punch.parse(org, this.user, 'in 9:15 pm', 'in');
            expect(punch).to.have.property('mode', 'in');
            expect(punch).to.have.deep.property('times[0]');
            const expectedTime = '09:15:00 PM';
            expect(punch.times[0].format('hh:mm:ss A')).to.equal(expectedTime);
        ...
        it('in at a full date', () => {
            const punch = Punch.parse(org, this.user, 'in 4pm 4/22/2016', 'in');
            expect(punch).to.have.property('mode', 'in');
            expect(punch).to.have.deep.property('times[0]');
        ...
        it('out at a time without minutes yesterday', () => {
            const punch = Punch.parse(org, this.user, 'out 7pm yesterday', 'out');
            expect(punch).to.have.property('mode', 'out');
            expect(punch).to.have.deep.property('times[0]');
        ...
        it('in at a 24-hour time and an existing project', () => {
            const punch = Punch.parse(org, this.user, `in 17:00 #${this.projectName}`, 'in');
            expect(punch).to.have.property('mode', 'in');
            expect(punch).to.have.deep.property('times[0]');
        ...
        it('append time block to punch', () => {
            const punch = Punch.parse(org, this.user, '1.5 hours');
        ...
        it('append time block to punch', () => {
            const punch = Punch.parse(org, this.user, '.5 hours');
            expect(punch).to.have.property('mode', 'none');
        ...
        it('append time block to punch', () => {
            const punch = Punch.parse(org, this.user, '0.5 hours');
            expect(punch).to.have.property('mode', 'none');
            expect(punch).to.have.deep.property('times.block', 0.5);
            expect(punch).to.have.property('projects');
            expect(punch.projects).to.be.empty;
        ...
        it('append time block to punch', () => {
            const punch = Punch.parse(org, this.user, '0.5 hour');
            expect(punch).to.have.property('mode', 'none');
            expect(punch).to.have.deep.property('times.block', 0.5);
            expect(punch).to.have.property('projects');
            expect(punch.projects).to.be.empty;
        ...
        it('append time block to punch', () => {
            const punch = Punch.parse(org, this.user, '1 hour');
            expect(punch).to.have.property('mode', 'none');
        ...
        it('append time block to yesterday\'s punch', () => {
            const punch = Punch.parse(org, this.user, '2 hours yesterday');
            expect(punch).to.have.property('mode', 'none');
        ...
        it('append time block to Tuesday\'s punch and assign it to an existing project', () => {
            const punch = Punch.parse(org, this.user, `3.25 hours tuesday ${this.projectName}`);
            expect(punch).to.have.property('mode', 'none');
        ...
        it('flag today as vacation', () => {
            const punch = Punch.parse(org, this.user, 'vacation today', 'vacation');
        ...
        it('flag half of today as sick', () => {
            const punch = Punch.parse(org, this.user, 'sick half-day', 'sick');
        ...
        it('flag half of yesterday as vacation', () => {
            const punch = Punch.parse(org, this.user, 'vacation half-day yesterday', 'vacation');
            expect(punch).to.have.property('mode', 'vacation');
            expect(punch).to.have.property('times');
            expect(punch).to.have.property('projects');
            expect(punch.projects).to.be.empty;
        ...
        it('flag a date range as sick', () => {
            const punch = Punch.parse(org, this.user, 'sick Jul 6-8', 'sick');
            expect(punch).to.have.property('mode', 'sick');
            expect(punch).to.have.property('times');
        ...
        it('flag a date range as vacation', () => {
            const punch = Punch.parse(org, this.user, 'vacation 1/28 - 2/4', 'vacation');
            expect(punch).to.have.property('mode', 'vacation');
            expect(punch).to.have.property('times');
            expect(punch).to.have.deep.property('times.length', 2);
    ...
    describe('#isValid(user)', () => {
        ...
        it('should return a failure reason for a repetitive in punch', () => {
            this.user.punches.push(Punch.parse(org, this.user, 'in', 'in'));
            const punch = Punch.parse(org, this.user, 'in', 'in');
            expect(punch.isValid(this.user)).to.be.a('string');
        });
        it('should return a failure reason for an in punch dated yesterday', () => {
            const punch = Punch.parse(org, this.user, 'in yesterday', 'in');
            expect(punch.isValid(this.user)).to.be.a('string');
        });
        it('should return a failure reason for an non-salary user punching unpaid time', () => {
            this.user.salary = false;
            const punch = Punch.parse(org, this.user, 'unpaid 9:50-10:00', 'unpaid');
            expect(punch.isValid(this.user)).to.be.a('string');
        });
        ...
        it('should return a failure reason for a sick punch that exceeds available sick time', () => {
            ...
            const punch = Punch.parse(org, this.user, 'sick today', 'sick');
            expect(punch.isValid(this.user)).to.be.a('string');
        });
        it('should return a failure reason for a vacation/sick/unpaid punch that isn\'t divisible by 4', () => {
            let punch = Punch.parse(org, this.user, 'vacation 9:50-10:00', 'vacation');
            expect(punch.isValid(this.user)).to.be.a('string');
            punch = Punch.parse(org, this.user, 'sick 9:50-10:00', 'sick');
            expect(punch.isValid(this.user)).to.be.a('string');
            punch = Punch.parse(org, this.user, 'unpaid 9:50-10:00', 'unpaid');
            expect(punch.isValid(this.user)).to.be.a('string');
        });
        it('should return a failure reason for any punch dated older than 7 days', () => {
            const punch = Punch.parse(org, this.user, 'in Jul 6-8', 'in');
            expect(punch.isValid(this.user)).to.be.a('string');
        });
