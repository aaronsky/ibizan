...
module.exports = (robot) ->
  ...
  # inform them that payroll runs on Monday, and that unaccounted-for time 
  # will not be paid.
  reminderJob = schedule.scheduleJob '0 13 * * 5', () ->
    # TODO: This should be a DM
    robot.sendMessage 'Payroll will run on Monday. Unrecorded time will not be paid'
...
# When given a command, Ibizan should always responds (in the channel) to confirm. 
  # When checking out or logging blocks of time, the entry should be read back along with the latest totals. 
  # e.g. 3.25 hours logged today / Today’s total: 5.5 hours / Week total: 40 hours